...
export default function Dashboard({ user, onLogout, onSwitchPortal }) {
  ...
  const metricCards = [
    ...
    { title: 'TOTAL STAFF', value: '84', change: '+3% vs last month', isUp: true, icon: Users, bg: 'bg-emerald-500', textColor: 'text-emerald-600' },
    { title: 'DEPARTMENTS', value: '6', change: '0% vs last month', isUp: true, icon: Building2, bg: 'bg-purple-500', textColor: 'text-purple-600' },
    { title: 'TOTAL COURSES', value: '24', change: '+2 vs last month', isUp: true, icon: BookOpen, bg: 'bg-indigo-500', textColor: 'text-indigo-600' },
    { title: 'TODAY\'S ATTENDANCE', value: '89.2%', change: '+1.4% vs last month', isUp: true, icon: CheckSquare, bg: 'bg-teal-500', textColor: 'text-teal-600' },
    ...
    { title: 'LEAVE REQUESTS', value: '7', change: '+3 vs last month', isUp: true, icon: Calendar, bg: 'bg-orange-500', textColor: 'text-orange-600' },
    { title: 'UPCOMING EXAMS', value: '3', change: 'this week vs last month', isUp: true, icon: FileSpreadsheet, bg: 'bg-rose-500', textColor: 'text-rose-600' }
  ];
  ...
  const recentActivities = [
    ...
    { title: 'Staff member added', detail: 'Dr. Neeraj Gupta – ECE', time: '1 hr ago', bg: 'bg-purple-100 text-purple-600', icon: Users },
    { title: 'Leave request submitted', detail: 'Prof. Ramesh Kumar', time: '2 hr ago', bg: 'bg-amber-100 text-amber-600', icon: Calendar },
    { title: 'Exam created', detail: 'Mid-Semester – CS 4th Sem', time: '3 hr ago', bg: 'bg-rose-100 text-rose-600', icon: FileText },
  ...
  const quickActions = [
    ...
    { label: 'Mark Attendance', icon: CheckSquare, bg: 'bg-emerald-50 hover:bg-emerald-100 text-emerald-700' },
    ...
    { label: 'Create Exam', icon: FileText, bg: 'bg-rose-50 hover:bg-rose-100 text-rose-700' },
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex font-sans">
      ...
      {/* LEFT SIDEBAR */}
      <aside className="w-64 bg-white border-r border-slate-200 flex flex-col justify-between shrink-0 sticky top-0 h-screen overflow-y-auto">
        <div>
          {/* Logo / Title Header */}
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11px] text-slate-400 italic hidden lg:inline">
                ← Click to demo different user portals
              </span>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {metricCards.map((card, idx) => {
              ...
              return (
                <div 
                  key={idx} 
                  className="bg-white border border-slate-200/80 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all duration-200 flex items-start justify-between"
                >
                  <div>
                  ...
                  <div className={`w-10 h-10 rounded-xl ${card.bg} text-white flex items-center justify-center shadow-sm shrink-0`}>
              ...
              <div className="grid grid-cols-2 gap-3">
                {quickActions.map((action, i) => {
                  ...
                  return (
                    <button 
                      key={i}
                      onClick={() => alert(`${action.label} action clicked!`)}
