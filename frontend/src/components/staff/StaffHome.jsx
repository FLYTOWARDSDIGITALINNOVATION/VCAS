...
export default function StaffHome({ staffUser, onNavigate }) {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:5000/api/classes/staff/${staffUser.staffId}`)
      .then(r => r.json())
      .then(d => { if (d.success) setDbClasses(d.classes || []); })
      .catch(() => {});

    fetch(`http://localhost:5000/api/attendance/staff/${staffUser.staffId}`)
      .then(r => r.json())
      .then(d => { if (d.success) setDbHistory(d.sessions || []); })
      .catch(() => {});

    fetch(`http://localhost:5000/api/timetable/staff/${staffUser.staffId}`)
      .then(r => r.json())
  ...
  const todayClasses = useMemo(() => {
    ...
    Object.entries(slots).forEach(([slot, cls]) => {
      if (cls && cls.code) {
        classesList.push({ ...cls, slot: Number(slot), time: slotTimes[slot] || `Period ${slot}` });
      }
    });
    return classesList.sort((a, b) => a.slot - b.slot);
  }, [dbTimetable, today]);
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
          ...
          <div className="p-4 space-y-2">
            {[
              ...
              { label: 'Internal Marks', icon: FileSpreadsheet, color: 'text-blue-700 bg-blue-50 hover:bg-blue-100 border-blue-100', nav: 'Internal Marks' },
              ...
              { label: 'View Timetable', icon: Calendar, color: 'text-indigo-700 bg-indigo-50 hover:bg-indigo-100 border-indigo-100', nav: 'Timetable' },
              { label: 'Apply Leave', icon: UserCheck, color: 'text-amber-700 bg-amber-50 hover:bg-amber-100 border-amber-100', nav: 'Leave' },
              { label: 'View Notices', icon: Bell, color: 'text-teal-700 bg-teal-50 hover:bg-teal-100 border-teal-100', nav: 'Notices' },
